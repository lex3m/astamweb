...
class RequestsController extends BackEndController
{
    ...
    public function accessRules()
    {
        return array(
            array('allow', // allow admin and moder user
                'actions'=>array('create','update', 'processRequests','index','view','admin','delete'),
	...
	public function actionProcessRequests()
    {
        if (isset($_POST['Requests']) && !empty($_POST['itemsSelected'])) {
            $type = Yii::app()->request->getPost('Requests');
            ...
            foreach($itemsSelected as $k => $v) {
                ...
                switch($type['workWithItemsSelected']) {
                    case 'new': $model->status = Requests::STATUS_NEW; $model->save();break;
                    case 'processed': $model->status = Requests::STATUS_READ; $model->save();break;
