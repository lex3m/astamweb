...
function editItem(obj){
    ...
    if($.isNumeric(pid)) {
        ...
        dlgStatus.notice(<?php echo CJavaScript::encode("Редактирование данного пункта невозможно"); ?>,<?php echo CJavaScript::encode("Ошибка"); ?>).open();
}
...
function create_node(e, data){
    ...
    if(typeof data.rslt.obj.parents("li:eq(0)").attr("pid")=='undefined'){
        dlgStatus.notice(<?php echo CJavaScript::encode("Родительский элемент не был создан. Повторите позже."); ?>,<?php echo CJavaScript::encode("Ошибка"); ?>).open();
        $.jstree.rollback(data.rlbk);
        return;
    }
    if(title==<?php echo CJavaScript::encode("Введите название пункта меню"); ?> || title == ''){
        $.jstree.rollback(data.rlbk);
        return;
    }
    jstree.jstree('lock');
    $.post(
        '<?php echo $this->createUrl("createNewItem"); ?>',
        {
            "parent_menu_id" : data.rslt.obj.parents("li:eq(0)").attr("pid"),
            "title" : title,
        ...
        .error(function(jqXHR){
            jstree.jstree('unlock');
            $.jstree.rollback(data.rlbk);
            dlgStatus.notice(jqXHR.responseText,<?php echo CJavaScript::encode("Ошибка"); ?>).open();
            jstree.jstree("refresh");
        });
};
//переименовать узел
function rename_node(e,data){
    ...
    dlgConfirm.confirm("<?php echo CJavaScript::encode("Вы действительно хотите переименовать"); ?> "+oldName+" <?php echo CJavaScript::encode("в"); ?> "+newName+" ",<?php echo CJavaScript::encode("Переименовать пункт меню"); ?>,
        function(){
            $.post(
...
//переключение чекбокса активности меню
...
function remove_node(e,data){
    dlgConfirm.confirm("<?php echo CJavaScript::encode("Вы дейстивительно хотите удалить этот пункт меню?"); ?> "+data.inst.get_text(data.rslt.obj)+" <?php echo CJavaScript::encode("и всех его потомков?"); ?>",<?php echo CJavaScript::encode("Удаление пункта меню"); ?>,
        function(){
            $.post(
...
function contextMenu(node){
    var items={
        create : {
            label	: <?php echo CJavaScript::encode("Создать внутри"); ?>,
            action: function(){jstree.jstree('create')}
        },
        rename : {
            label	: <?php echo CJavaScript::encode("Переименовать"); ?>,
            action: function(){jstree.jstree('rename')}
        },
        remove : {
            label	: <?php echo CJavaScript::encode("Удалить"); ?>,
            action: function(){jstree.jstree('remove')}
        },
        edit : {
            label: <?php echo CJavaScript::encode("Редактировать"); ?>,
...
$(function(){
    ...
    jstree=$("#pageList").jstree({
        core: {
            "strings": {
                new_node: <?php echo CJavaScript::encode("Введите название пункта меню"); ?>,
                loading: <?php echo CJavaScript::encode("Загрузка ..."); ?>
            }
        },
        json_data: {
            "ajax": {
